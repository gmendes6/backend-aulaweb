        ...
        client.query(
            ...
            (err, result) => {
                ...
                if (result.rowCount == 0) {
                    res.send("Não há usuários com o código: " + req.params.id)
                }
                res.send(result.rows);
                //console.log(result);
            }
    ...
    try {
        ...
        client.query(
            "DELETE FROM Usuarios WHERE id = $1", [req.params.id], (err, result) => {
                if (err) {
                    res.send("Ocorreu um erro na pesquisa");
                    return console.error("Erro ao executar a qry de DELETE", err);
                } else {
                    if (result.rowCount == 0) {
                        res.status(404).json({ info: "Registro não encontrado." });//not found
                    } else {
